...
MIN_EVAL_ROWS = 8  # Minsta antal rader i träning och validering.
...
def build_features(
    ...
    if (
        out.empty
        or out["price"].isna().any()
        or not np.isfinite(out["price"]).all()
        or (out["price"] <= 0).any()
    ):
    ...
    if macro_df is not None:
        out = out.merge(
            build_macro_features(out["date"], macro_df),
            on="date",
            how="left",
            validate="one_to_one",
        )
...
def _training_window(rows, weeks):
    """Begränsa historiken utifrån det senaste tillåtna träningsexemplets datum.

    Faktorer byggs före filtreringen. Ett år räknas som 52 veckor.
    """
...
def price_interval_diagnostics(actual_price, predicted_price) -> dict:
    ...
    valid = (
        actual.size > 0
        and actual.shape == predicted.shape
    ...
    calibration_errors = (
        (calibration_actual - calibration_predicted) / calibration_predicted * 100
    )
    ...
    if coverage_count and margin is not None:
        later_errors = np.abs(
            (actual[calibration_count:] - predicted[calibration_count:])
...
def _three_way_split(feat: pd.DataFrame, val_size: float, test_size: float, horizon_weeks: int):
    """Dela kronologiskt i träning, validering och test utan överlappande mål.

    Reservera en horisonts marginal före nästa period. Returnera None om
    historiken är för kort, så att anroparen kan använda en train/test-delning.
    """
    n = len(feat)
    ...
    train = feat.iloc[:val_start]
    validation = feat.iloc[val_start:test_start]
    test = feat.iloc[test_start:]

    # Varje mål måste vara känt innan nästa period börjar.
    train = train.loc[train["target_date"] < validation["date"].iloc[0]]
    validation = validation.loc[validation["target_date"] < test["date"].iloc[0]]
    if len(train) < MIN_EVAL_ROWS or len(validation) < MIN_EVAL_ROWS or len(test) < 2:
        return None
...
def train_model(
    ...
    if not 0 < val_size < 1 or not 0 < test_size < 1 or val_size + test_size >= 1:
        raise ValueError(
            "val_size och test_size måste vara mellan 0 och 1 och summera till mindre än 1."
        )

    feature_columns = _feature_columns(macro_df)
    features = build_features(df, horizon_weeks, macro_df).dropna(
        subset=feature_columns + ["target_return"]
    )
    if len(features) < 3:
        raise ValueError("För lite komplett historik för vald modell och horisont.")

    split = _three_way_split(features, val_size, test_size, horizon_weeks)
    ...
    if split is not None:
        ...
        best_params, history_weeks, best_val_rmse, validation_scores = _select_configuration(
            method, train, validation, feature_columns
        )
        train_for_eval = pd.concat([train, validation])
        n_train = len(_training_window(train, history_weeks))
        n_val = len(validation)
        tuned = True
    else:
        test_rows = max(int(len(features) * test_size), 2)
        test_start = len(features) - test_rows
        train_for_eval = features.iloc[:test_start]
        ...
        best_params = best_val_rmse = None
        n_train = len(train_for_eval)
        n_val = 0
        tuned = False

    # Utvärdera vald konfiguration en gång på den separata testperioden.
    train_for_eval = _training_window(train_for_eval, history_weeks)
    production_rows = _training_window(features, history_weeks)
...
def predict_price(
    ...
    feature_columns = _feature_columns(macro_df)
    features = build_features(df, horizon_weeks, macro_df)
    latest = features.iloc[[-1]]
    if latest[feature_columns].isna().any().any():
        raise ValueError(
            "Prognosen kräver minst 53 veckopriser och kompletta, aktuella data för alla valda faktorer."
        )
...
if __name__ == "__main__":
    for horizon_label, weeks in FORECAST_HORIZONS.items():
        ...
        for method_name in METHODS:
            trained_model, metrics = train_model(method=method_name, horizon_weeks=weeks)
            price = predict_price(model=trained_model, horizon_weeks=weeks)
            val_rmse_str = f"{metrics['val_rmse']:>10,.0f}" if metrics["tuned"] else "     (ej tunad)"
            print(
                f"  {method_name:20s} val-RMSE {val_rmse_str}  "
                f"test-RMSE {metrics['rmse']:>12,.0f} (naiv {metrics['naive_rmse']:>12,.0f})  "
                f"params {metrics['best_params']}  prognos: {price:>12,.0f}"
            )
